"""
Experiment: Learn Sigma and Beta Parameters with Noisy Observations using State Augmented EnKF

This experiment extends learn_sigma_perfect_enkf by:
- Learning TWO parameters instead of one: sigma and beta
- Using NOISY observations instead of perfect observations
- Keeping rho fixed at its true value

This tests the State Augmented EnKF's ability to learn multiple parameters
from realistic noisy data using an ensemble of particles.
...
Goal: Assess parameter identifiability and learning speed when:
- Learning from noisy observations (realistic scenario)
- Learning multiple parameters simultaneously
- Testing if rho can be kept fixed while learning sigma and beta
"""
...
def run_experiment(config: DictConfig) -> Dict:
    ...
    print("\n" + "="*70)
    print("EXPERIMENT: Learn Sigma and Beta Parameters using State Augmented EnKF")
    print("="*70)
    ...
    # Learning sigma (index 0) and beta (index 2), keeping rho (index 1) fixed
    ...
    theta0_ensemble = theta0_ensemble + torch.randn_like(theta0_ensemble) * (
        torch.tensor([0.5, 0.0, 0.3])  # Spread for sigma, no spread for rho (fixed), spread for beta
    )
...
def plot_results(results: Dict, output_dir: Path) -> None:
    ...
    # Figure 1: Sigma and Beta parameter convergence (ensemble statistics)
    ...
    ax.set_title('Sigma Parameter Convergence - State Augmented EnKF (with Noisy Observations)', 
    ...
    ax.scatter([0], [initial_beta], color='orange', s=100, marker='o', 
    ...
    ax.set_title('Beta Parameter Convergence - State Augmented EnKF (with Noisy Observations)', 
                 fontsize=13, fontweight='bold')
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(output_dir / 'sigma_beta_convergence.png', dpi=150)
    plt.close()
    ...
    # Figure 3: Parameter history for all ensemble members (sigma and beta only)
    ...
    fig.suptitle('Parameter Evolution - Sigma and Beta (All Ensemble Members)', fontsize=14, fontweight='bold')
...
def plot_trajectory_comparison(results: Dict, output_dir: Path) -> None:
    ...
    # Get rho from true params (fixed)
    ...
    for label, sigma_val, beta_val in [
        ...
        traj = [x0.numpy()]
        X = x0.clone()
        params = torch.tensor([[sigma_val, rho, beta_val]])
    ...
    for i, (ax, name) in enumerate(zip(axes, ['x', 'y', 'z'])):
        ...
        ax.plot(time_steps, trajectories['true'][:, i], 'r--', linewidth=2, 
                label=f'True (σ={true_sigma:.4f}, β={true_beta:.4f})', alpha=0.7, zorder=3)
        ax.plot(time_steps, trajectories['initial'][:, i], 'orange', 
                linestyle=':', linewidth=2, 
                label=f'Initial (σ={initial_sigma:.4f}, β={initial_beta:.4f})', 
                alpha=0.7, zorder=2)
        ax.plot(time_steps, trajectories['final'][:, i], 'b--', linewidth=2, 
                label=f'Final (σ={final_sigma:.4f}, β={final_beta:.4f})', alpha=0.7, zorder=3)
    ...
    fig.suptitle('Trajectory Comparison: Learning Sigma and Beta from Noisy Observations (EnKF)', 
